...
if (isset($_GET['delete'])) {
    ...
    // Don't allow deleting users with comments or favorites
    $check_comments = $db->query("SELECT COUNT(*) as c FROM comments WHERE user_id = $id")->fetch_assoc()['c'];
    $check_favorites = $db->query("SELECT COUNT(*) as c FROM user_favorites WHERE user_id = $id")->fetch_assoc()['c'];
    
    if ($check_comments > 0 || $check_favorites > 0) {
        setFlashMessage('error', 'User tidak bisa dihapus karena memiliki aktivitas (komentar/favorit). Nonaktifkan saja.');
    } else {
                        ...
                        <tbody class="divide-y divide-gray-200">
                            <?php if ($users_result->num_rows > 0): ?>
                                <?php while ($user = $users_result->fetch_assoc()): ?>
                                    <?php
                                    // Get user activity
                                    $comments_count = $db->query("SELECT COUNT(*) as c FROM comments WHERE user_id = {$user['id']}")->fetch_assoc()['c'];
                                    ...
                                    <tr class="hover:bg-gray-50 transition">
                                        ...
                                        <td class="px-6 py-4 text-sm text-gray-600">
                                            <div class="space-y-1">
                                                <p>💬 <?php echo $comments_count; ?> komentar</p>
